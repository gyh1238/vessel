...
GOAL_NORM_K = 150.0
GOAL_REACHED = 3.0
COMM_RANGE = float(os.environ.get('VESSEL_COMM_RANGE', '200.0'))  # 정책 통신 파트너 반경 (관측 채널)
# ★통신 반경 ≠ 보상 반경. 보상은 DETECTION_RANGE(레이더)와 정합 — OFF privileged reward 금지(2026-09-10).
# ★2026-08-27: 하드코딩 420 → env. config.py 가 같은 VESSEL_COMM_RANGE 를 읽음.
...
# ★보상이 반응하는 반경.
#   DETECTION_RANGE = 규정·상황 판정 거리(레이더와 맞춤).
#   ★2026-09-10: 학습/평가는 reward_range=DETECTION_RANGE 고정(OFF 관측 정합).
#   2026-08-30 COMM_RANGE 확장은 OFF privileged reward 를 만들어 Fig1을 오염시킴 → 폐기.
#   VESSEL_REWARD_RANGE 로만 실험 override (기본 생성자 경로).
REWARD_RANGE = float(os.environ.get('VESSEL_REWARD_RANGE', str(DETECTION_RANGE)))
...
ARRIVAL_REWARD = float(os.environ.get('VESSEL_ARRIVAL_REWARD', '150.0'))  # ★reward#1 iter2: 100→150; 재균형 시 env로 완화 가능
TIMEOUT_PENALTY = -50.0       # ★2026-08: timeout=실패 → 완만 페널티(충돌 -300보다 훨씬 약해 충돌 유발 안 함, 배회 억제)
# ★전방 근접벌점 계수(장애물·벽·타선 레이더). reward#1 기본 -1.0(제곱). 안전↑ 시 |coef|↑.
PROX_COEF = float(os.environ.get('VESSEL_PROX_COEF', '-1.0'))
...
# ★PRIMARY 정렬 추가 shaping (2026-09-06): eval Rule8/17 과 학습 신호를 맞춤. 0=off(비트동일).
#   Give-way: 목표항로 대비 우현 오프셋·감속 보상 / Stand-on: 침로·타각 유지 보상.
COLREGS_PRIMARY_ALIGN = float(os.environ.get('VESSEL_COLREGS_PRIMARY_ALIGN', '0'))
...
class VesselBatchEnv:
    ...
    def _reward(self, actions, radar):
        ...
        # ★reward#1 fix: 근접벌점 계수↓(-2.0→-1.0)+제곱 → 중간거리 통과 허용, 접촉부근만 강함.
        #   VESSEL_PROX_COEF 로 재균형(예: -2.0 → 접촉부근 fmin=0 에서 약 -2).
        _pnorm = torch.clamp(1 - fmin / THR, min=0.0)
        prox = torch.where(fmin < THR, PROX_COEF * _pnorm * _pnorm, torch.zeros_like(fmin))
        r = r + prox
        ...
        if COLREGS_MODE in ('unity', 'unity_cs'):
            ...
            # ★PRIMARY-align: eval(colregsOK)과 같은 Rule8/17 행동을 매 스텝 강화
            if COLREGS_PRIMARY_ALIGN > 0.0:
                _to_g = self.goal - self.pos
                _gbrg = torch.atan2(_to_g[..., 0], _to_g[..., 1]) / DEG   # deg, Unity xz
                _off = torch.remainder(self.heading - _gbrg + 180.0, 360.0) - 180.0  # + = 우현으로 goal선에서 벗어남
                _gw_m = ((_sit == 1) | (_sit == 3) | (_sit == 4)) & (_cgate > 0)
                # Rule 8 course: 우현 오프셋 ~10° 를 향해 soft 보상 (0°→0, 10°→1, 캡 1.5)
                _off_n = torch.clamp(_off.clamp(min=0.0) / 10.0, 0.0, 1.5)
                _pri = torch.where(_gw_m, 0.40 * _off_n, torch.zeros_like(r))
                # Rule 8 speed: 위험 게이트 안 감속
                _pri = _pri + torch.where(_gw_m & (speed_ratio < 0.85),
                                         torch.full_like(r, 0.25), torch.zeros_like(r))
                # Rule 17: early stand-on 구간에서 타각·오프셋 모두 작으면 보상
                _so_m = (_sit == 2) & (_cgate > 0) & (_early17 > 0)
                _pri = _pri + torch.where(
                    _so_m & (_nr.abs() < 0.1) & (_off.abs() < 20.0),
                    torch.full_like(r, 0.40), torch.zeros_like(r))
                r = r + (COLREGS_PRIMARY_ALIGN * COLREGS_SIM_COEF) * _pri * _riskw
        else:
            _rud = actions[..., 0]
