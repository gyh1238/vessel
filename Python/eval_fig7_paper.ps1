...
$csv = Join-Path $Out "mixed_fleet_rx_v2.csv"
$ep = Join-Path $Out "mixed_fleet_rx_v2_episodes.csv"

foreach ($s in $Seeds) {
  $ckpt = Join-Path $Paper "ckpts\final\qd_MOE_SE_s$s.pt"
  if (-not (Test-Path $ckpt)) {
    Write-Host "MISSING hub $ckpt — train -Fig hub first"
    continue
  }
  Write-Host "Fig7 seed=$s mode=$Mode sweep=$Sweep"
