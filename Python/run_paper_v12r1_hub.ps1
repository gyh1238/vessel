# v12r1 WIN hub: Fig4 DIM8/10/12 + Fig1 OFF, residual+L2. NEVER writes ckpts/final.
# Fig2 SHARED 3-seed already in runs/paper/v12r1; do not retrain those.
param(
  [int[]]$Gpus = @(0, 1, 2, 3),
  [int]$Steps = 16000000,
  [int]$Envs = 128
...
$V12r = Join-Path $Paper "v12r1_hub"
...
$ov = @{ VESSEL_MOE_SHARE_BACKBONE = "1"; VESSEL_MOE_RESIDUAL_HEAD = "1"; VESSEL_MOE_DELTA_L2 = "0.01" }
foreach ($s in 42, 43, 44) {
  $pending.Add([pscustomobject]@{
      Kind = "train"; Pri = 10; Tag = "q_DIM8_RL2_s$s"; Arm = "ON"; Seed = $s
      Override = $ov; MsgDim = 8; Crossing = 0; CommOnAt = 9000000; Label = "q_DIM8_RL2_s$s"
    })
  $pending.Add([pscustomobject]@{
      Kind = "train"; Pri = 11; Tag = "q_DIM10_RL2_s$s"; Arm = "ON"; Seed = $s
      Override = $ov; MsgDim = 10; Crossing = 0; CommOnAt = 9000000; Label = "q_DIM10_RL2_s$s"
    })
  $pending.Add([pscustomobject]@{
      Kind = "train"; Pri = 12; Tag = "q_DIM12_RL2_s$s"; Arm = "ON"; Seed = $s
      Override = $ov; MsgDim = 12; Crossing = 0; CommOnAt = 9000000; Label = "q_DIM12_RL2_s$s"
    })
  $pending.Add([pscustomobject]@{
      Kind = "train"; Pri = 20; Tag = "qf_SE_OFF_RL2_s$s"; Arm = "OFF"; Seed = $s
      Override = $ov; MsgDim = 6; Crossing = 0; CommOnAt = 0; Label = "qf_SE_OFF_RL2_s$s"
    })
}
...
function Enqueue-PostEval($job) {
  ...
  $pending.Insert(0, [pscustomobject]@{
      ...
      Crossing = $job.Crossing; Partners = 4; Decisions = 3000; MsgDim = $job.MsgDim
...
function Start-JobOnGpu($job, [int]$gpu) {
  ...
  if ($job.Kind -eq "eval") {
    ...
    $p = Start-Process -FilePath $Py -ArgumentList @(
      ...
      "--ring", "0.7", "--crossing", "$($job.Crossing)", "--max_partners", "4", "--device", "cuda:0"
  ...
  $argList.AddRange([string[]]@(
      ...
      "--seed", "$($job.Seed)", "--max_partners", "4", "--comm_on_at", "$($job.CommOnAt)",
  ...
  if ($ckpt) {
    ...
    } else { Write-Status "FRESH train $($job.Tag) gpu=$gpu dim=$($job.MsgDim) arm=$($job.Arm)" }
  } else { Write-Status "FRESH train $($job.Tag) gpu=$gpu dim=$($job.MsgDim) arm=$($job.Arm)" }
...
Write-Status "v12r1 hub Fig4 DIM8/10/12 + Fig1 OFF start gpus=$($Gpus -join ',')"
try {
  ...
  Write-Status "v12r1 hub campaign complete"
} catch {
