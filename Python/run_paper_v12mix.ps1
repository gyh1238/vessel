# Fig2 only: v12p MoE + train soft route-mix=0.15 (eval hard). vs SINGLE.
# Rare heads get gradient. NOT residual. NEVER FINAL.
param(
  [int[]]$Gpus = @(0, 1, 2, 3),
  [int]$Steps = 16000000,
  [int]$Envs = 128
...
foreach ($s in 42, 43, 44) {
  $pending.Add([pscustomobject]@{
      Kind = "train"; Pri = 10; Tag = "qd_MOE_SE_MX_s$s"; Arm = "ON"; Seed = $s
      Override = $ovShare; Crossing = 0; CommOnAt = 9000000; Label = "qd_MOE_SE_MX_s$s"
    })
  $pending.Add([pscustomobject]@{
      Kind = "train"; Pri = 11; Tag = "q_MOE_SINGLE_MX_s$s"; Arm = "ON"; Seed = $s
      Override = $ovSingle; Crossing = 0; CommOnAt = 9000000; Label = "q_MOE_SINGLE_MX_s$s"
    })
}
...
function Enqueue-PostEval($job) {
  ...
  $pending.Insert(0, [pscustomobject]@{
      ...
      Crossing = $job.Crossing; Partners = 4; Decisions = 3000
...
function Start-JobOnGpu($job, [int]$gpu) {
  ...
  if ($ckpt) {
    ...
    } else { Write-Status "FRESH train $($job.Tag) gpu=$gpu arm=$($job.Arm)" }
  } else { Write-Status "FRESH train $($job.Tag) gpu=$gpu arm=$($job.Arm)" }
...
Write-Status "v12mix Fig2 route-mix=0.15 MoE vs SINGLE gpus=$($Gpus -join ',') win=Fig2"
try {
  ...
  Write-Status "judging Fig2 route-mix"
  & $Py -u (Join-Path $Root "judge_v12mix.py")
  Write-Status "v12mix campaign complete"
} catch {
