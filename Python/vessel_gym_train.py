...
def main():
    ...
    # ★2026-09-10 fix (Fig1 정합): 보상 반경 = DETECTION_RANGE(레이더/상황 관측 가능 거리).
    #   2026-08-30 에 reward_range=COMM_RANGE 로 올려 "통신 밴드에도 비용"을 걸었으나, OFF arm 은
    #   56m 밖 파트너를 *관측할 수 없는데* 같은 보상이 걸려 privileged reward → Fig1 불공정.
    #   공유 보상은 OFF 관측과 정합해야 함. 통신 이득 = 메시지로 56m 진입 전 조율 → goal/충돌 개선.
    #   farfield PBRS 기본 OFF. VESSEL_FARFIELD_COEF 부활 시 arm별 관측 정합 재검토.
    env = vg.VesselBatchEnv(num_envs=E, n_vessels=N, device=device, seed=args.seed,
                            ring_scale=args.ring, crossing=args.crossing,
                            risk_range=cfg.COMM_RANGE, reward_range=vg.DETECTION_RANGE,
    ...
    try:
        while total_decisions < args.steps:
            ...
            if csv_f:
                ...
                # ★매 update flush: 20-update 완충이면 모니터가 '멈춤'으로 오인함
                #   (실측: 1.31M CSV 고정처럼 보였으나 그 사이 step2M.pt 는 정상 저장됨).
                csv_f.flush()
            if aux_f and _sr_log.get('_n', 0) > 0:
                ...
                _sr_log = {}
                aux_f.flush()
            # heartbeat: 외부 모니터용 (CSV와 별도로 마지막 step/시각)
            if args.save:
                try:
                    with open(os.path.splitext(args.save)[0] + '.heartbeat', 'w', encoding='utf-8') as _hf:
                        _hf.write(f"{total_decisions}\t{time.time():.0f}\t{args.arm}\t{args.seed}\n")
                except OSError:
                    pass
            if update_i % 5 == 0:
