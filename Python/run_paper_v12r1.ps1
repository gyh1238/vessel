# Residual-head SHARED + Δμ L2. NEVER writes ckpts/final.
# Trains qd_MOE_SE_RL2_s42/43/44 vs existing v12 SINGLE_B.
param(
  [int[]]$Gpus = @(0, 1, 2),
  [int]$Steps = 16000000,
  [int]$Envs = 128
...
foreach ($s in 42, 43, 44) {
  $pending.Add([pscustomobject]@{
      Kind = "train"; Pri = 10; Tag = "qd_MOE_SE_RL2_s$s"; Arm = "ON"; Seed = $s
      Override = $ov; MsgDim = 6; Crossing = 0; CommOnAt = 9000000; Label = "qd_MOE_SE_RL2_s$s"
    })
}
...
function Start-JobOnGpu($job, [int]$gpu) {
  ...
  if ($ckpt) {
    ...
    } else { Write-Status "FRESH train $($job.Tag) gpu=$gpu residual=1" }
  } else { Write-Status "FRESH train $($job.Tag) gpu=$gpu residual=1" }
...
Write-Status "v12r1 residual+L2 campaign start gpus=$($Gpus -join ',')"
try {
  ...
  Write-Status "judging residual+L2 vs SINGLE_B"
  & $Py -u (Join-Path $Root "judge_v12r1.py")
  Write-Status "v12r1 campaign complete"
} catch {
